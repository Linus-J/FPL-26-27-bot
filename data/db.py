...
def _rebuild_team_matches_on_code() -> None:
    """Drop a pre-``team_code`` ``team_matches`` before ``create_all`` runs
    (2026-09-06).

    Must run BEFORE ``create_all``: the table shipped keyed on ``team_id``
    (``teams.id``, reassigned every season), and ``create_all`` never alters
    an existing table, so a database that already has the old shape would
    keep it forever. Dropping first lets the same ``create_all`` call below
    rebuild it keyed on ``team_code`` in one startup.
    ...
    """
    from data.migrations import rebuild_team_matches_on_code

    try:
        rebuild_team_matches_on_code(engine)
    except Exception as exc:  # noqa: BLE001 -- startup must never die on cleanup
        logger.warning("team_matches rebuild-on-code skipped: %s", exc)
